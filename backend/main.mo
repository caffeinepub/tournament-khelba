...
// Run migration on actor upgrade
...
actor {
  ...
  public query ({ caller }) func listPendingPayments() : async [Payment] {
    if (not (AccessControl.hasPermission(accessControlState, caller, #admin))) {
  ...
  public query ({ caller }) func listPaymentsByTournament(tournamentId : Nat) : async [Payment] {
    if (not (AccessControl.hasPermission(accessControlState, caller, #admin))) {
